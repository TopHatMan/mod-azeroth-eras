...
- Ready target still: Vanilla 1.0–1.6
...
world DB
  item_template       era snapshots from the module (1.0-1.6 first)
...
File-based `dbc/vanilla` vs `dbc/tbc` vs `dbc/wotlk` is optional later. Do **not** put ARAC files in those era folders or a 1.0 boot will lose all-races-all-classes.

## Recommended order

1. Keep 1.0–1.6 **gates** as the playable target (already the ready target).
2. Turn off DBC item-attribute enforcement in this module while a Vanilla patch is selected.
3. Build `item_template` snapshots for 1.0–1.6 (vMaNGOS → AC fields). That is itemization.
